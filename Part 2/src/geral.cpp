...
Viagem*  adicionaViagem(Bilheteira *b, Frota *f, Paragens *p) {
	...
	while (id_comboio < 0 || id_comboio >= f->getNumComboios() || f->getComboio(id_comboio)->getAvariado()) {
		...
		if (f->getComboio(id_comboio)->getAvariado())
			cout << "Este Comboio esta em manunten�ao na oficina: " << f->getComboio(id_comboio)->getOficina()->getNome();
...
void menuParagens(Paragens *p) {
	...
	while (menu != 4) {
		while (menu == -2 || menu == -1) {
			...
			cout << " 3 - Mostrar as paragens ordenadas por número de clientes" << endl;
			cout << " 4 - Sair " << endl;
		...
		case 2: {
			...
			if (par >= p->sizeParagens() || par < 0){
				cout << endl << "Paragem n�o existe" << endl;
				return;
			}
			p->deleteParagem(par);
	...
	while (menu != 3) {
		...
		case 1:
		{
			...
			if (ofi >= o->sizeOficinas() || ofi < 0){
				cout << endl << "Oficina n�o existe" << endl;
			}
